    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-sm mx-4 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl p-6 space-y-5">
        ...
        <div className="text-center space-y-1.5">
          ...
          <h2 className="text-lg font-semibold text-zinc-100">Sign in to Consensus AI</h2>
          <p className="text-xs text-zinc-500">Save your chats and access them from any device</p>
        </div>
        ...
        {emailSent ? (
          <div className="text-center space-y-2 py-4">
            ...
            <p className="text-xs text-zinc-500">We sent a magic link to <span className="text-zinc-300">{email}</span></p>
            <button onClick={() => setEmailSent(false)} className="text-xs text-indigo-400 hover:text-indigo-300 mt-2">
          ...
          <>
            ...
            <div className="space-y-2.5">
              <Button
                ...
                className="w-full bg-white hover:bg-zinc-100 text-zinc-900 font-medium gap-2 h-10"
              >
                {loading === 'google' ? (
              ...
              <Button
                ...
                disabled={loading !== null}
                variant="outline"
                className="w-full border-zinc-700 text-zinc-200 hover:bg-zinc-800 gap-2 h-10"
              >
                {loading === 'github' ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
            ...
            <form onSubmit={handleEmail} className="space-y-2.5">
              <input
                ...
                required
                className="w-full px-3 py-2.5 rounded-lg bg-zinc-800 border border-zinc-700 text-sm text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-indigo-500 transition-colors"
              />
              <Button
                type="submit"
